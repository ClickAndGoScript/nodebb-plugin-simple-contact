{{{ if !activity.length }}}
<div style="color:#aaa; font-size:13px; text-align:center;">[[simple-contact:admin.no-activity]]</div>
{{{ end }}}
{{{ each activity }}}
<div style="padding:5px 0; border-bottom:1px solid #f0f0f0; font-size:13px;">
	...
	{{{ if ./assigned }}}
	[[simple-contact:admin.activity.assigned]]
	{{{ if ./toUsername }}}<strong>{./toUsername}</strong>{{{ end }}}
	{{{ if !./toUsername }}}<strong>[[simple-contact:admin.not-assigned]]</strong>{{{ end }}}
	{{{ end }}}
	{{{ if !./assigned }}}
	{{tx(./labelKey)}}
	{{{ end }}}
